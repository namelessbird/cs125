...
export default function Login({ setU }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Persist user ID so App.js can grab it on refresh
            localStorage.setItem("bookApp_userId", userID);
            setU(userID);
        ...
        <div className="bg-[#384264] h-screen flex flex-col items-center justify-center">
            <form onSubmit={handleSubmit} className="relative flex flex-col text-center content-center bg-white rounded-md px-8 pt-20 pb-10 sm:w-[30%] sm:max-w-96 sm:min-w-64">
                ...
                <label className="sm:-mx-2">
                    <input 
                        className="border-gray-200 border-2 rounded pl-2 py-1 sm:w-[100%]" 
                        type="text" 
                ...
                <button type="submit" className="text-white bg-[#384264] cursor-pointer py-1.5 mx-[25%] rounded-md hover:brightness-80">
                    Log In
                </button>
            ...
            {/* Changed <a> to <Link> to keep React State alive */}
            <Link to="/register" className="text-white cursor-pointer underline pt-2.5">
